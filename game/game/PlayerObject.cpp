...
PlayerObject::PlayerObject( PlayerInfo *player, Script *script, unsigned long type ) : AnimatedObject( script->GetStringData( "mesh_name" ), script->GetStringData( "mesh_path" ), type )
{
	...
	m_currentWeapon = 0;
	if( player->dpnid == Engine::GetInstance()->GetNetwork()->GetLocalID() )
...
void PlayerObject::Update( float elapsed, bool addVelocity )
{
	...
	if( m_changingWeapon > 0.0f )
	{
		...
		else if( m_changingWeapon < 0.0f )
		{
			...
			pwcm.dpnid = Engine::GetInstance()->GetNetwork()->GetLocalID();
			pwcm.weapon = m_currentWeapon;
			Engine::GetInstance()->GetNetwork()->Send( &pwcm, sizeof( PlayerWeaponChangeMsg ), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK );
		}
		else
		{
...
void PlayerObject::Render( D3DXMATRIX *world )
{
	...
	if( m_dpnid != Engine::GetInstance()->GetNetwork()->GetLocalID() )
...
void PlayerObject::Hurt( float damage, PlayerObject *attacker )
{
	...
	phm.dpnid = m_dpnid;
	phm.health = m_health;
	Engine::GetInstance()->GetNetwork()->Send( &phm, sizeof( PlayerHealthMsg ), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK );
	...
	Engine::GetInstance()->GetNetwork()->Send( &psm1, sizeof( PlayerScoreMsg ), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK );
	...
	Engine::GetInstance()->GetNetwork()->Send( &psm2, sizeof( PlayerScoreMsg ), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK );
}
...
void PlayerObject::ChangeWeapon( char change, char weapon )
{
	...
	pwcm.dpnid = Engine::GetInstance()->GetNetwork()->GetLocalID();
	Engine::GetInstance()->GetNetwork()->Send( &pwcm, sizeof( NetworkMessage ), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK );
}
