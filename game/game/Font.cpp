...
Font::Font( char *name, short size, unsigned long bold, bool italic )
{
	...
	if( FAILED( Engine::GetInstance()->GetDevice()->CreateTexture( m_textureWidth, m_textureHeight, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &m_texture, NULL ) ) )
		goto End;
	...
	Engine::GetInstance()->GetDevice()->CreateVertexBuffer( 1020 * sizeof( TLVertex ), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &m_vb, NULL );

	// Prepare the alpha testing for rendering the characters.
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_ALPHAREF, 0x08 );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATEREQUAL );

	// Create a state block for capturing the volatile render states.
	Engine::GetInstance()->GetDevice()->BeginStateBlock();
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_LIGHTING, false );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_ALPHATESTENABLE, false );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, false );
	Engine::GetInstance()->GetDevice()->EndStateBlock( &m_states );
...
void Font::Render( char *text, float x, float y, D3DCOLOR colour )
{
	...
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_LIGHTING, false );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_ALPHATESTENABLE, true );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, false );

	// Set the non-volatile render states.
	Engine::GetInstance()->GetDevice()->SetTexture( 0, m_texture );
	Engine::GetInstance()->GetDevice()->SetFVF( TL_VERTEX_FVF );
	Engine::GetInstance()->GetDevice()->SetStreamSource( 0, m_vb, 0, TL_VERTEX_FVF_SIZE );
		...
		if( c != _T( ' ' ) )
		{
			...
			*vertices++ = TLVertex( D3DXVECTOR4( x - 0.5f, y - 0.5f, 0.0f, 1.0f ), colour, tx1, ty1 );
			...
			if( totalTriangles == 340 )
			{
				...
				m_vb->Unlock();
				Engine::GetInstance()->GetDevice()->DrawPrimitive( D3DPT_TRIANGLELIST, 0, totalTriangles );
				vertices = NULL;
				m_vb->Lock( 0, 0, (void**)&vertices, D3DLOCK_DISCARD );
				totalTriangles = 0;
			}
