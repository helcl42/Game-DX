...
void SceneManager::LoadScene( char *name, char *path )
{
	...
	Engine::GetInstance()->GetDevice()->SetLight( 0, &sun );
	Engine::GetInstance()->GetDevice()->LightEnable( 0, true );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_SPECULARENABLE, true );

	// Set up the fog.
	float density = *script->GetFloatData( "fog_density" ) * m_scale;
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGENABLE, true );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGCOLOR, D3DCOLOR_COLORVALUE( script->GetColourData( "fog_colour" )->r, script->GetColourData( "fog_colour" )->g, script->GetColourData( "fog_colour" )->b, script->GetColourData( "fog_colour" )->a ) );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGVERTEXMODE, D3DFOG_EXP2 );
	Engine::GetInstance()->GetDevice()->SetRenderState( D3DRS_FOGDENSITY, *(unsigned long*)&density );
	...
	m_mesh = Engine::GetInstance()->GetMeshManager()->Add( script->GetStringData( "mesh" ), script->GetStringData( "mesh_path" ) );
	...
	display = Engine::GetInstance()->GetDisplayMode();
	...
	Engine::GetInstance()->GetDevice()->SetTransform( D3DTS_PROJECTION, &projection );
	...
	for( unsigned long m = 0; m < m_mesh->GetStaticMesh()->NumMaterials; m++ )
	{
		...
		if( found == false && m_mesh->GetStaticMesh()->materials[m]->GetIgnoreFace() == false )
			m_renderCaches->Add( new RenderCache( Engine::GetInstance()->GetDevice(), m_mesh->GetStaticMesh()->materials[m] ) );
	}
	...
	if( NeedDefaultMaterial == true )
		m_renderCaches->Add( new RenderCache( Engine::GetInstance()->GetDevice(), Engine::GetInstance()->GetMaterialManager()->Add( "defaultMaterial" ) ) );
	...
	Engine::GetInstance()->GetDevice()->CreateVertexBuffer( m_totalVertices * VERTEX_FVF_SIZE, D3DUSAGE_WRITEONLY, VERTEX_FVF, D3DPOOL_MANAGED, &m_sceneVertexBuffer, NULL );
...
void SceneManager::Render( float elapsed, D3DXVECTOR3 viewer )
{
	...
	Engine::GetInstance()->GetDevice()->SetStreamSource( 0, m_sceneVertexBuffer, 0, VERTEX_FVF_SIZE );
	Engine::GetInstance()->GetDevice()->SetFVF( VERTEX_FVF );
