...
Weapon::Weapon( Script *script, D3DXVECTOR3 viewWeaponOffset )
{
	...
	strcpy_s(m_name, strlen(m_name) * sizeof(char), script->GetStringData( "name" ));
	...
	m_velocity = *script->GetFloatData( "muzzle_velocity" ) / Engine::GetInstance()->GetScale();
	m_damage = *script->GetFloatData( "max_damage" );
	m_range = *script->GetFloatData( "range" ) / Engine::GetInstance()->GetScale();
...
//		sprintf( flash, "flash%d", m_flashes->GetTotalElements() );
		sprintf_s( flash, "flash%d", sizeof(char) * 16, m_flashes->GetTotalElements() );
	}
...
void Weapon::Update( float elapsed, bool fire, SceneObject *parent, float viewPointY )
{
	...
	if( m_fireTimer > m_rof )
	{
		...
		Game::GetInstance()->GetBulletManager()->AddBullet( parent, ( (PlayerObject*)parent )->GetEyePoint(), ( (PlayerObject*)parent )->GetForwardVector(), m_velocity, m_range, m_damage );
...
void Weapon::Render( SceneObject *parent )
{
	...
	if( m_displayFlash == true )
	{
		...
		Engine::GetInstance()->GetDevice()->GetTransform( D3DTS_VIEW, &view );
		Engine::GetInstance()->GetSprite()->SetWorldViewLH( &world, &view );
		Engine::GetInstance()->GetSprite()->Begin( D3DXSPRITE_BILLBOARD | D3DXSPRITE_ALPHABLEND );
		Engine::GetInstance()->GetSprite()->Draw( material->GetTexture(), NULL, &D3DXVECTOR3( material->GetWidth() / 2.0f, material->GetHeight() / 2.0f, 0.0f ), &m_muzzelPoint, 0xFFFFFFFF );
