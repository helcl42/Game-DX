...
void Game::Close()
{
	...
	Engine::GetInstance()->GetNetwork()->Terminate();
...
void Game::HandleNetworkMessage( ReceivedMessage *msg )
{
	...
	switch( msg->msgid )
	{
		case MSGID_CREATE_PLAYER:
		{
			...
			PlayerObject *object = m_playerManager->AddPlayer( Engine::GetInstance()->GetNetwork()->GetPlayer( msg->dpnid ) );
			Engine::GetInstance()->GetSceneManager()->AddObject( object );

			// Check if the new player is the host player.
			if( object->GetID() == Engine::GetInstance()->GetNetwork()->GetHostID() )
			{
				// Load the scene from the host player's selection.
				Engine::GetInstance()->GetSceneManager()->LoadScene( ( (PlayerData*)Engine::GetInstance()->GetNetwork()->GetPlayer( msg->dpnid )->data )->map, "./Assets/Scenes/" );

				// Allow the network to receive game specific messages.
				Engine::GetInstance()->GetNetwork()->SetReceiveAllowed( true );
			}
		...
		case MSGID_DESTROY_PLAYER:
		{
			...
			Engine::GetInstance()->GetSceneManager()->RemoveObject( &object );
		...
		case MSGID_SPAWN_POINT_REQUEST:
		{
			...
			spm.dpnid = msg->dpnid;
			spm.spawnPoint = Engine::GetInstance()->GetSceneManager()->GetSpawnPointID( Engine::GetInstance()->GetSceneManager()->GetRandomPlayerSpawnPoint() );
			Engine::GetInstance()->GetNetwork()->Send( &spm, sizeof( SpawnPointMsg ), msg->dpnid );
		...
		case MSGID_SPAWN_POINT:
		{
			...
			if( spm->dpnid != Engine::GetInstance()->GetNetwork()->GetLocalID() )
				break;
