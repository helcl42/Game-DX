...
void UpdateSessionsList( HWND window )
{
	...
	Engine::GetInstance()->GetNetwork()->EnumerateSessions();
	...
	char name[MAX_PATH];
	SessionInfo *session = Engine::GetInstance()->GetNetwork()->GetNextSession( true );
	while( session != NULL )
	{
		...
		session = Engine::GetInstance()->GetNetwork()->GetNextSession();
	}
		...
		case WM_COMMAND:
		{
			switch( LOWORD( wparam ) )
			{
				case IDC_HOST:
				{
					...
					if( Engine::GetInstance()->GetNetwork()->Host( name, session, 8, &data, sizeof( data ) ) )
					{
						Engine::GetInstance()->ChangeState( STATE_GAME );
				...
				case IDC_JOIN:
				{
					...
					if( Engine::GetInstance()->GetNetwork()->Join( name, session, &data, sizeof( data ) ) )
					{
						Engine::GetInstance()->ChangeState( STATE_GAME );
...
void Menu::Update( float elapsed )
{
	...
	DialogBox( NULL, MAKEINTRESOURCE( IDD_MENU ), Engine::GetInstance()->GetWindow(), MenuDialogProc );
}